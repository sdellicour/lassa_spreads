...
# B7. Prepration of skygrid-GLM covariates
# B8. Analysis of skygrid-GLM analyses
...
nberOfExtractionFiles = 1000
	...
	# To do:
		# - alternative plotting: a single map for all three NGA clades? (With a single colour code/clade)
		# - (in this alternative plotting:) remove the root and isolate tip for the NIG1 clade
		# - (in this alternative plotting:) remove the uncertainty related to the root of the MRU clade
		# - prepare two different sets of "seraphim" analyses (with low and high resolution rasters)
...
for (i in 1:length(segments))
	{	
		localTreesDirectory = paste0("LASV_",segments[i],"_align_3_all_extractions")
...
for (i in 1:length(segments))
	{
		...
		# 4.2.5. Dividing extraction files per genotype (clade)

source("isolateACladeFromTipNodes.r")
for (i in 1:length(segments))
	{
		localTreesDirectory1 = paste0("LASV_",segments[i],"_align_3_all_extractions")
		...
		for (j in 1:length(clades))
			{
				...
				files = list.files("LASV_80HPD_polygons_20yrs"); pols = list(); cols_pol = list()
				files = files[which((grepl(paste0("_",segments[i],"_"),files))&(grepl(clades[j],files))&(grepl(".shp",files)))]
				for (k in 1:length(files))
					{
						pol = shapefile(paste0("LASV_80HPD_polygons_20yrs/",files[k]))
...
setwd(paste(wd,wdb6,sep="/"))
	...
	# 6.1. Generating null models of lineages dispersal
		...
		# 6.1.1. Randomisation of tree branches position (randomisation procedure n°3)

for (i in 1:length(study_areas))
	{
		rast = raster(paste0("Environmental_files/LASV_rasters/Elevation_",study_areas[i],"_0.008.asc")); rast[!is.na(rast[])] = 0
...
for (i in c(1,3,5,6)) # check-up:
	...
	# 6.2. Analysing the impact of rivers on the dispersal frequency of lineages
...
analyses = c("LASV_segment_L_MRU","LASV_segment_L_NGA","LASV_segment_S_MRU","LASV_segment_S_NGA")

	# 7.1. Preparation of climatic covariates
...
prefixes = c("Temperature","Precipitation")
yLabs = c("annual mean temperature","annual precipitation")
for (h in 1:length(variables))
	{
		if (!file.exists(paste0("CRU_",variable_names[h],"_data.csv")))
			{
				nc = nc_open(paste0("CRU_",variable_names[h],"_data.nc"))
				...
				write.csv(var2, paste0("CRU_",variable_names[h],"_data.csv"), row.names=F, quote=F)
			}	else		{
				var2 = read.csv(paste0("CRU_",variable_names[h],"_data.csv"), header=T)
			}
		...
		for (i in 1:length(analyses))
			{
				...
				colnames(values2) = c("time",variable_names[h])
				write.csv(values2, paste0(prefixes[h],gsub("LASV_segment_","_seg",analyses[i]),".csv"), row.names=F)
			}
		if (h == 1)
			{
				...
				axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(24.4,28.2,0.4))
				...
				axis(side=1, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,-0.05,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(1900,2020,10))
				axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(25.6,28.0,0.4))
				title(xlab="time (year)", cex.lab=0.6, mgp=c(0.7,0,0), col.lab="gray30")
				title(ylab=yLabs[h], cex.lab=0.6, mgp=c(1.1,0,0), col.lab="gray30")
				title(main="Nigeria", cex.main=0.8, col.main="gray30", line=-0.1)
				dev.copy2pdf(file=paste0("CRU_",variable_names[h],"_data.pdf"))
			}
		if (h == 2)
			{
				dev.new(width=9, height=3); par(mfrow=c(1,2), mgp=c(0,0,0), oma=c(1.0,1.0,0.5,0.5), mar=c(2.0,2.0,1,1))
				...
				lines(curves2[[1]][,1], curves2[[1]][,2], lwd=1.0, col="red")
				axis(side=1, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,-0.05,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(1900,2020,10))
				axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(100,220,20))
				title(xlab="time (year)", cex.lab=0.6, mgp=c(0.7,0,0), col.lab="gray30")
				title(ylab=yLabs[h], cex.lab=0.6, mgp=c(1.1,0,0), col.lab="gray30")
				title(main="M-R-U", cex.main=0.8, col.main="gray30", line=-0.1)
				legend(1900, 115, legend=c("sliding window of 5 years"), lwd=1, cex=0.6, col=c("red"), text.col="gray30", border=NA, x.intersp=0.5, bty="n")
				plot(curves1[[3]][,1], curves1[[3]][,2], lwd=0.5, type="l", axes=F, ann=F)
				lines(curves2[[3]][,1], curves2[[3]][,2], lwd=1.0, col="red")
				axis(side=1, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,-0.05,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(1900,2020,10))
				axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(80,150,10))
				title(xlab="time (year)", cex.lab=0.6, mgp=c(0.7,0,0), col.lab="gray30")
				title(ylab=yLabs[h], cex.lab=0.6, mgp=c(1.1,0,0), col.lab="gray30")
				title(main="Nigeria", cex.main=0.8, col.main="gray30", line=-0.1)
				dev.copy2pdf(file=paste0("CRU_",variable_names[h],"_data.pdf"))
	...
	# 7.2. Preparation of land cover covariates
...
curves_list = list(); selected_years = c(1901:2016)
variable_names = c("crops","pasture","urban land","forested primary land","non-forested primary land",
				   "potentially forested secondary land","potentially non-forested secondary land")
prefixes = c("Croplands","Pastures","UrbanLand","PrimaryForest","PrimaryNonF","SecondaryForest","SecondaryNonF")
cols = c("navajowhite4","burlywood3","#8856A7","chartreuse4",NA,"olivedrab3",NA)
for (i in 1:length(variable_names))
	{
		...
		colnames(variable3) = c("longitude","latitude",selected_years); curves = list()
		for (j in 1:length(analyses))
			{
				...
				curves[[j]] = values; colnames(values) = c("time",gsub("-","_",gsub(" ","_",variable_names[i])))
				write.csv(values, paste0(prefixes[i],gsub("LASV_segment_","_seg",analyses[j]),".csv"), row.names=F)
			}
...
for (i in 1:length(variable_names))
	{
		...
		for (j in 1:length(analyses))
			{
				curves[[j]] = read.csv(paste0(prefixes[i],gsub("LASV_segment_","_seg",analyses[j]),".csv"), header=T)
			}
		curves_list[[i]] = curves
	}

dev.new(width=9, height=3); par(mfrow=c(1,2), mgp=c(0,0,0), oma=c(1.0,1.0,0.5,0.5), mar=c(2.0,2.0,1,1))
plot(curves_list[[1]][[1]][,1], curves_list[[1]][[1]][,2], lwd=1.5, type="l", axes=F, ann=F, col=cols[1], ylim=c(0,0.6))
for (i in c(2,3,4,6)) lines(curves_list[[i]][[1]][,1], curves_list[[i]][[1]][,2], lwd=1.5, col=cols[i])
axis(side=1, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,-0.05,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(1900,2020,10))
axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(0,1,0.1))
title(xlab="time (year)", cex.lab=0.6, mgp=c(0.7,0,0), col.lab="gray30")
title(ylab="land use proportion", cex.lab=0.6, mgp=c(1.1,0,0), col.lab="gray30")
title(main="M-R-U", cex.main=0.8, col.main="gray30", line=-0.1)
plot(curves_list[[1]][[2]][,1], curves_list[[1]][[2]][,2], lwd=1.5, type="l", axes=F, ann=F, col=cols[1], ylim=c(0,0.6))
for (i in c(2,3,4,6)) lines(curves_list[[i]][[2]][,1], curves_list[[i]][[2]][,2], lwd=1.5, col=cols[i])
axis(side=1, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,-0.05,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(1900,2020,10))
axis(side=2, lwd.tick=0.2, cex.axis=0.5, mgp=c(0,0.25,0), lwd=0.2, tck=-0.020, col.axis="gray30", at=seq(0,1,0.1))
title(xlab="time (year)", cex.lab=0.6, mgp=c(0.7,0,0), col.lab="gray30")
title(ylab="land use proportion", cex.lab=0.6, mgp=c(1.1,0,0), col.lab="gray30")
title(main="Nigeria", cex.main=0.8, col.main="gray30", line=-0.1)
dev.copy2pdf(file=paste0("Land_use_proportions_NEW.pdf"))

entropies = list()
for (i in 1:length(analyses))
	{
		proportions = matrix(nrow=dim(curves_list[[1]][[i]])[1], ncol=length(variable_names)+1)
		...
		colnames(entropy) = c("time","entropy"); entropies[[i]] = entropy
		write.csv(entropy, paste0("LUH_entropy",gsub("LASV_segment_","_seg",analyses[i]),".csv"), row.names=F)
	}

	# 7.3. Preparation of the BEAST skygrid-GLM analyses
...
mostRecentSamplingDatum = decimal_date(ymd("2019-03-07"))
for (i in 1:length(analyses))
	{
		segment = unlist(strsplit(analyses[i],"_"))[3]
		studyArea = unlist(strsplit(analyses[i],"_"))[4]
		covariates = list(); dates = list(); ylims = list(); covariate_names = rep(NA, 7)
		covariates[[1]] = read.csv(paste0("Temperature_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[2]] = read.csv(paste0("Precipitation_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[3]] = read.csv(paste0("PrimaryForest_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[4]] = read.csv(paste0("SecondaryForest_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[5]] = read.csv(paste0("Croplands_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[6]] = read.csv(paste0("Pastures_seg",segment,"_",studyArea,".csv"), header=T)
		covariates[[7]] = read.csv(paste0("UrbanLand_seg",segment,"_",studyArea,".csv"), header=T)
		for (j in 1:length(covariates)) covariate_names[j] = colnames(covariates[[j]])[2]
		gridPoints = round(mostRecentSamplingDatum-rev(1902:floor(mostRecentSamplingDatum)),4)
		gridPoints_txt = gridPoints[1]
		for (j in 2:length(gridPoints))
			{
				gridPoints_txt = paste(gridPoints_txt,gridPoints[j],sep=" ")
			}
		...
		for (j in 1:dim(timePoints)[1])
			{
				timePoint = as.integer(timePoints[j,1])
				for (k in 1:length(covariates))
					{
						if (sum(timePoint==as.numeric(covariates[[k]][,1])) == 1)
							{
								timePoints[j,covariate_names[k]] = covariates[[k]][which(covariates[[k]][,1]==timePoint),2]
							}
					}
			}
		...
		if (length(covariates) > 1)
			{
				for (j in 2:length(covariates))
					{
						# txt = c(txt, paste0("\t\t\t<parameter id=\"skygrid.lastObservedIndex.",j,"\" value=\"",length(gridPoints)+1,"\"/>"))
					}
			}
		# txt = c(txt, paste0("\t\t</lastObservedIndex>"))
		...
		for (j in 1:length(covariates))
			{
				maxDiff = 0
				for (k in 2:dim(timePoints)[1])
					{
						if ((!is.na(as.numeric(timePoints[k,covariate_names[j]])))&(!is.na(as.numeric(timePoints[k-1,covariate_names[j]]))))
							{
								if (maxDiff < abs(as.numeric(timePoints[k,covariate_names[j]])-as.numeric(timePoints[k-1,covariate_names[j]])))
									{
										maxDiff = abs(as.numeric(timePoints[k,covariate_names[j]])-as.numeric(timePoints[k-1,covariate_names[j]]))
									}
							}
					}
				values = as.numeric(timePoints[,covariate_names[j]]); values = values[!is.na(values)]
				covPrec = 1/((sd(values))^2)
				# txt = c(txt, paste0("\t\t\t<parameter id=\"skygrid.covPrec.",j,"\" value=\"",covPrec,"\"/>"))
			}
		# txt = c(txt, paste0("\t\t</covariatePrecision>"))
		txt = c(txt, "")
		...
		for (j in 1:length(covariates))
			{
				txt = c(txt, paste0("\t\t\t<parameter id=\"skygrid.beta.",j,"\" value=\"0\" lower=\"-1000\" upper=\"1000\"/>"))
			}
		txt = c(txt, paste0("\t\t</betaParameter>"))
		txt = c(txt, "")
		...
		for (j in 1:length(covariates))
			{
				c = 0; lastValue = "NA"; meanValue = mean(as.numeric(timePoints[,covariate_names[j]]), na.rm=T)
				txt = c(txt, paste0("\t\t\t<matrixParameter id=\"",covariate_names[j],"\">"))
				for (k in dim(timePoints)[1]:1)
					{
						c = c+1
						if (!is.na(timePoints[j,covariate_names[j]]))
							{
								txt = c(txt, paste0("\t\t\t\t<parameter id=\"cov",j,".",c,"\" value=\"",timePoints[k,covariate_names[j]],"\"/>"))
								lastValue = timePoints[k,covariate_names[j]]
							}	else	{
								startValue = timePoints[max(which(!is.na(timePoints[,covariate_names[j]]))),covariate_names[j]]
								txt = c(txt, paste0("\t\t\t\t<parameter id=\"cov",j,".",c,"\" value=\"",startValue,"\" lower=\"0\"/>"))
							}
					}
		...
		for (j in 1:length(covariates))
			{
				...
				txt = c(txt, paste0("\t\t\t<parameter idref=\"skygrid.beta.",j,"\"/>"))
		...
		txt = c(txt,"","XXXXXXXXXXXXXXXXXX","")
		for (j in 1:length(covariates)) txt = c(txt, paste0("\t\t\t<parameter idref=\"skygrid.beta.",j,"\"/>"))
		txt = c(txt, paste0("\t\t\t<parameter idref=\"skygrid.gridPoints\"/>"))
		write(txt, paste0("LASV_",segment,"_",studyArea,"_GLM_block.xml"))
	}
